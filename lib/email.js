...
<body style="margin: 0; padding: 0; font-family: 'Arial', sans-serif; background-color: #000000; color: #ffffff;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    <div style="text-align: center; padding: 20px; color: #6b7280; font-size: 12px;">
      <p>Questions? Contact us at <a href="mailto:support@cicada.com" style="color: #a855f7;">support@cicada.com</a></p>
      <p style="margin-top: 10px;">© 2025 Cicada Music Society. All rights reserved.</p>
    ...
    const { data, error } = await resend.emails.send({
      from: "Cicada Music Society <onboarding@resend.dev>", // Using Resend onboarding address
      to: [to],
      subject: `Your ${
        ticketArray.length > 1 ? `${ticketArray.length} Tickets` : "Ticket"
...
export async function sendConfirmationEmail({
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: "Cicada Music Society <noreply@cicada.com>",
      to: [to],
      subject: "Order Confirmation - Cicada Music Society",
      html: `
